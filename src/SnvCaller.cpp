...
std::vector<VcfRecord> fetchSnvs(const std::string& inputVcf, const std::string& region) {
    std::vector<VcfRecord> snvs;
    VcfReader reader(inputVcf);
    reader.SetRegion(region);

    bcf_hdr_t* hdr = reader.getHeader();
    if (!hdr) {
        std::cerr << " ERROR: VCF header is null for file: " << inputVcf << std::endl;
        return snvs;
    }
    std::vector<VcfRecord> records;
    while (reader.GetNextRecord(records)) {
        snvs.insert(snvs.end(), 
