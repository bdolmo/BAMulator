...
void BamWriter::WriteRecord(const BamRecord& record) {
    ...
    // Ensure it has RG tag (skip if already present)
    if (bam_aux_get(b, "RG") == nullptr) {
        std::string rg_id = "RB16187";
        bam_aux_append(b, "RG", 'Z', rg_id.length() + 1, reinterpret_cast<const uint8_t*>(rg_id.c_str()));
    }
